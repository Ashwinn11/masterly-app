...
const MaterialTape = ({ type, index }: { type: string; index: number }) => {
  ...
  return <div className={cn("washi-tape w-24 h-8", tapeColor)} />;
};
...
export default function MaterialsPage() {
  ...
  const loadMaterials = useCallback(async () => {
    ...
    const { data: materialsData, error: materialsError } = await (
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 pt-8">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="h-64 border-crayon bg-white/50 animate-pulse rounded-2xl"
              />
            ))}
          </div>
        ) : materials.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-center space-y-6">
            ...
            <Button
              size="lg"
              className="border-crayon h-16 px-8 text-xl font-handwritten font-black"
              asChild
            >
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center text-center space-y-6 pt-4 relative z-10">
                        ...
                        <div className="space-y-2">
                          <h3 className="text-2xl font-black font-handwritten line-clamp-2 text-foreground leading-tight px-2 overflow-hidden break-words">
                            {material.title}
                          </h3>
                      ...
                      <div className="pt-6 border-t border-foreground/5 relative z-10 flex justify-between items-center">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="font-handwritten text-sm font-bold text-info uppercase tracking-widest">
                            Active
                          </span>
                        </div>
                        <span className="font-handwritten text-sm font-bold text-info opacity-40">
                          #{material.id.slice(0, 4)}
                        </span>
                      </div>
                    </div>
                  </Link>
